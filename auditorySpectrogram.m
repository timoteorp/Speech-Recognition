...
%
%   S = audioexample.auditorySpectrogram(x,fs) returns a mel-spaced
%   auditory spectrogram using default properties. x must be a single
%   channel (column vector) of type double or single. fs must be a real
%   positive scalar of type double or single.
%
%   S = audioexample.auditorySpectrogram(...,'WindowLength',WINDOWLENGTH)
%   specifies the analysis window length. Specify the window length in
%   samples as a positive scalar. If unspecified, WINDOWLENGTH defaults to
%   round(fs.*0.030).
%
%   S = audioexample.auditorySpectrogram(...,'OverlapLength',OVERLAPLENGTH)
%   specifies the number of samples overlap between adjacent windows.
%   Specify the overlap length as a positive scalar smaller than the window
%   length. If unspecified, OVERLAPLENGTH defaults to round(fs*0.02).
%
%   S = audioexample.auditorySpectrogram(...,'NumBands',NUMBANDS) specifies
%   the number of bandpass filters used. If unspecified, NUMBANDS defaults
%   to 40.
%
%   S = audioexample.auditorySpectrogram(...,'FFTLength',FFTLENGTH)
%   specifies the number of points in the DFT. If unspecified, FFTLENGTH
%   defaults to 512.
%
%   S = audioexample.auditorySpectrogram(...,'Range',RANGE) specifies the
%   start and end of the bandpass filters. If unspecified, RANGE defaults
%   to [0,floor(fs/2)].
%
%   S = audioexample.auditorySpectrogram(...,'WindowType',WINDOWTYPE)
%   specifies the window type applied before taking the FFT. Specify
%   WINDOWTYPE as 'Hann','Hamming', or 'Rectangular'. If unspecified,
%   WINDOWTYPE defaults to 'Hamming'.
%
%   S = audioexample.auditorySpectrogram(...,'SumExponent',SUMEXPONENT)
%   specifies the exponent of the frequency domain.
%
%   S = audioexample.auditorySpectrogram(...,'FilterBankNormalization',
%   FILTERBANKNORMALIZATION) specifies the type of filter bank 
%   nomalization. Specify FILTERBANKNORMALIZATION as 'Area', 'Bandwidth', 
%   or 'None'. If unspecified, FILTERBANKNORMALIZATION defaults to 
%   'Bandwidth'.
%
%   S = audioexample.auditorySpectrogram(...,'FilterBankDesignDomain',
%   FILTERBANKDESIGNDOMAIN) specifies the filter bank design domain. 
%   Specify FILTERBANKDESIGNDOMAIN as 'Bin' or 'Hertz'. The default is 
%  'Bin'.
%
%   S = audioexample.auditorySpectrogram(...,'WarpType',WARPTYPE) specifies
%   the warping algorithm for the filter bank bandedges. Specify WARPTYPE
%   as 'Bark','ERB', or 'Mel'. If unspecified, WARPTYPE defaults to 'Mel'.
%
%   audioexample.auditorySpectrogram(...) plots the spectrogram.
%
%   %EXAMPLE 1: Plot the Auditory Spectrogram of a Voice File
%     [audio,fs] = audioread('Counting-16-44p1-mono-15secs.wav');
%     audioexample.auditorySpectrogram(audio,fs)
%
...
if ~isequal(params,previousParams) || ~isequal(nRow,previousFrameSize)
    if ~isequal(params,previousParams)
        ...
        pFilterBank = audio.internal.designMelFilterBank(params.SampleRate, ...
            bandEdges, ...
            params.FFTLength, ...
            params.SumExponent, ...
...
function userInput = parseInputs(x,fs,varargin)
    ...
    if designParser == true
        ...
        parser.FunctionName    = 'audioexample.auditorySpectrogram';
